...
for iop in outcome_patterns:
    ...
    tmp_ydf['td'] = (tmp_ydf['tstop'] - tmp_ydf['tstart'])/np.timedelta64(1, 'D')
    tmp_ydf['td'] = tmp_ydf['td'].astype(int)
    tmp_ydf['tm'] = (tmp_ydf['tstop'] - tmp_ydf['tstart'])/np.timedelta64(1, 'M')
    tmp_ydf['tm'] = tmp_ydf['tm'].astype(int)
    tmp_ydf['ty'] = (tmp_ydf['tstop'] - tmp_ydf['tstart'])/np.timedelta64(1, 'Y')
    tmp_ydf['ty'] = tmp_ydf['ty'].astype(float)
...
for iop in outcome_patterns:
    ...
    tmp_df = tmp_df.dropna(axis=0, how='all')
    tmp_max = max(tmp_df.max())
    tmp_v00_surv = pd.DataFrame(columns=['ID', 'vtime', 'event', 'tstart', 'tstop', 'diff', 'vstart', 'vstop'], index = tmp_df.index)
    for idx in tmp_df.index.values.tolist():
        f0 = True
        for col in sorted(tmp_df.columns.values.tolist()):
