...
ann_xlsx = "AllClinical00_V6_column_annotation.xlsx"
...
all_col_oi = pd.read_excel(mainDir+'/'+ann_xlsx, sheet_name='life_activity_accelo', index_col='Variables')
print(all_col_oi)
...
for icol in col_oi:
    ...
    #                print(direct_merge_df)
                ...
                if 'SIDE' in tmp_data_1.columns.values.tolist():
                    ...
                    if len(ol_pid) > 0:
                        if tmp_data_1.shape[0] == direct_merge_df.shape[0]:
                            direct_merge_df = pd.concat([direct_merge_df, tmp_data_1[icol]], axis=1)
                        if tmp_data_2.shape[0] == direct_merge_df.shape[0]:
                            direct_merge_df = pd.concat([direct_merge_df, tmp_data_2[icol]], axis=1)
